...
public class CommentDAO {
	...
	// 글의 갯수 구하기
	public int getListCount(int comment_qna_number) {
		Connection con = null;
		...
		String sql = " select count(*) "
				   + " from qnacomm "
				   + " where comment_qna_number = ? ";
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, comment_qna_number);
	...
	public JsonArray getCommentList(int comment_qna_number, int state) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sort="asc";
		...
		String sql = "select num, qnacomm.id, content, reg_date, comment_re_lev, "
				+ "	   		 comment_re_seq, "
				+ "          comment_re_ref"
				+ "   from   qnacomm join member "
				+ "   on     qnacomm.id=member.member_id "
				+ "   where  comment_qna_number = ? "
	...
	public int commentsInsert(Comment c) {
		...
		try {
			...
			String sql = "insert into qnacomm "
			...
			pstmt.setInt(3, c.getComment_qna_number());
	...
	public int commentsUpdate(Comment co) {
		...
		try {
			con = ds.getConnection();			
			String sql = "update qnacomm set content=? "
					   + "where num = ? ";
			pstmt = con.prepareStatement(sql);
	...
	public int commentsDelete(int num) {
		...
		try {
			con = ds.getConnection();			
			String sql = "delete qnacomm where num = ? ";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			result = pstmt.executeUpdate();
			if(result == 1)
	...
	public int commentsReply(Comment c) {
		...
		try {
			...
			update_sql.append("update qnacomm ");
			...
			e.printStackTrace();
			System.out.println("commentReply() 에러: " + e);
		}finally {
            if (pstmt != null)
               try {
                  pstmt.close();
